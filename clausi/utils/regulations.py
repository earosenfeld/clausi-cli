...
def get_regulations(api_url: Optional[str] = None, use_cache: bool = True) -> Dict[str, Any]:
    ...
    try:
        response = requests.get(
            ...
            timeout=5  # Short timeout to avoid blocking CLI
        )
        ...
        # Silently fall back to cached or fallback regulations
        pass

    # Check cache one more time (maybe it's stale but better than nothing)
    cached = _get_cache()
    if cached:
        return cached
